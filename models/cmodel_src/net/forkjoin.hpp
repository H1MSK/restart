...
#pragma HLS STABLE variable = enable_grad
#pragma HLS DATAFLOW
        hls::stream<cm_float, Node1Type::in_size> in_x1;
        hls::stream<cm_float, Node2Type::in_size> in_x2;
        hls::stream<cm_float, Node1Type::out_size> out_y1;
        hls::stream<cm_float, Node2Type::out_size> out_y2;
...
#pragma HLS INTERFACE mode = ap_fifo port = out_grad_x
#pragma HLS DATAFLOW
        hls::stream<cm_float, Node1Type::in_size> out_grad_x1;
        hls::stream<cm_float, Node2Type::in_size> out_grad_x2;
        hls::stream<cm_float, Node1Type::out_size> in_grad_y1;
        hls::stream<cm_float, Node2Type::out_size> in_grad_y2;
...
// I don't know exactly the behave of the if statement in HLS.
//  If it will be precalculated and converted into content in either branch
//  when compiling, then dataflow will be applied as my intention.
     void backward(hls::stream<cm_float>& in_grad_y) {
 #pragma HLS INTERFACE mode = ap_ctrl_chain port = return
 #pragma HLS INTERFACE mode = ap_fifo port = in_grad_y
 #pragma HLS DATAFLOW
         ...
         hls::stream<cm_float, Node2Type::out_size> in_grad_y2;
         joiner.backward2(in_grad_y, in_grad_y1, in_grad_y2);
         if constexpr(ForkerType::param_size != 0) {
             node1.backward(in_grad_y1, out_grad_x1);
             node2.backward(in_grad_y2, out_grad_x2);
