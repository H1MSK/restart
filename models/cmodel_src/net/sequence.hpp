...
struct Sequence<FirstType, OtherTypes...> : public Sequence<OtherTypes...> {
...
#pragma HLS INLINE
        first.dumpGradAndZero(out_grad);
        SeqNext::dumpGradAndZero(out_grad);
    }
#endif
...
#pragma HLS DATAFLOW
        hls::stream<cm_float> mid;
        first.forward(in_x, mid, enable_grad);
        SeqNext::forward(mid, out_y, enable_grad);
    }
...
#pragma HLS DATAFLOW
        hls::stream<cm_float> mid;
        SeqNext::backward(in_grad_y, mid);
