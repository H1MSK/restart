...
def gen_cache_tools_source(cache_debugger_filename):
    with open(cache_debugger_filename, "w") as f:
        f.write(load_template("cache_tools_source.cpp").substitute(
            ...
            cache_writter_cases="\n    ".join(
                f"case {i+1}: {c.name} << val; break;" for i, c in enumerate(dag.all_caches())
            ),
            cache_reader_cases="\n    ".join(
                f"case {i+1}: *val = {c.name}.read(); break;" for i, c in enumerate(dag.all_caches())
            ),
            cache_monitor_inputs=", ".join(f"int {c.name}_in" for c in dag.all_caches()),
            cache_monitor_outputs=", ".join(f"int *{c.name}_out" for c in dag.all_caches()),
